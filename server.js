...
wss.on("connection", (twilioWs) => {
  ...
  oaWs.on("message", (raw) => {
    ...
    if (evt.type === "conversation.item.input_audio_transcription.completed") {
      ...
      if (text.includes("bye") || text.includes("adios") || text.includes("adiós") || text.includes("gracias")) {
        console.log("👋 Detectada despedida. Colgando...");
        setTimeout(() => {
          if (streamSid) {
            client.calls(streamSid).update({status: 'completed'}).catch(e => console.log("Call already closed"));
          }
          twilioWs.close();
        }, 2000);
      }
  ...
  twilioWs.on("close", async () => {
    if (fullTranscript.length > 2) {
      ...
      // Extracción de Datos para los técnicos
      const phone = chat.match(/(\d[\s-]?){7,12}/g)?.pop()?.replace(/\s/g, '') || "❌ No capturado";
      const name = chat.match(/Cliente: (?:My name is|Soy) (.*)/i)?.[1] || "Ver chat";
      const address = chat.match(/(?:\d+\s+[\w\s]+(?:Street|St|Ave|Avenue|Drive|Dr|Road|Rd|Way|Lane|Ln|Boulevard|Blvd))/i)?.[0] || "Ver chat";
...
server.listen(PORT, '0.0.0.0', () => console.log(`🚀 Elena v28.0 Dispatcher Ready`));
